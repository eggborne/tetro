function update() {
  ...
  if (titleScreen.container.visible && counter === 0) {
    ...
    if (Date.now() % 2 === 0) {
      ...
      if (
        Math.abs(titleScreen.castleContainer.rotation) >
        (Math.PI / 360) * 5
      ) {
        titleScreen.castleContainer.rotation -=
          (Math.PI / 360) * randRotation * 2;
      }
      if (
        Math.abs(titleScreen.castleContainer.x - window.innerWidth / 2) >
        titleScreen.castleContainer.width / 2
      ) {
        titleScreen.castleContainer.x -= randRotation;
      }
  ...
  if (lost < 0) {
    ...
    if (
      survivalLevelSelectScreen &&
      survivalLevelSelectScreen.container.visible
    ) {
    }
    ...
    if (
      survivalLevelSelectScreen &&
      survivalLevelSelectScreen.container.fading
    ) {
    ...
    if (atBat) {
        ...
        if (
          ...
          atBat.shift(
            -(startedDragLocation.x - touchingAt.x) /
              Math.abs(startedDragLocation.x - touchingAt.x),
            -(startedDragLocation.x - touchingAt.x) /
              Math.abs(startedDragLocation.x - touchingAt.x)
          );
          startedDragLocation.x = touchingAt.x;
      ...
      if (
        counter !== 0 &&
        counter - atBat.born > atBat.fallSpeed &&
        atBat.born !== counter &&
        atBat.born !== counter - 1 &&
        counter !== atBat.born + atBat.fallSpeed &&
        (counter - atBat.born) % atBat.fallSpeed === 0
      ) {
        if (
          gameMode !== 'survival' &&
          gameMode !== 'timed' &&
          (atBat.forced || atBat.dropped)
        ) {
          forceBonus++;
        }
